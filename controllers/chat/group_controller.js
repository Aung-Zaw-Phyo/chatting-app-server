...
exports.getGroups = async(req, res, next) => {
    try {
        ...
        res.status(201).json({message: 'Successfully fetched users.', data: {groups: groups}})
    } catch (error) {
        next(error)
    }
}
...
exports.searchGroup = async (req, res, next) => {
    try{
        ...
        const groups = await Group.find({
            "$or": [
                { name: { $regex: search, $options: 'i' }, creator: userId },
                { name: { $regex: search, $options: 'i' }, members: userId },
            ]
        }).populate(['creator', 'members'])
        res.status(200).json({status: true, message: 'Successfully fetched groups', data: {groups}})
    } catch (error) {
        next(error)
    }  
}
...
exports.searchMember = async (req, res, next) => {
    try {
        ...
        const user = req.user
        res.status(200).json({status: true, message: 'Successfully fetched user', data: {user: user}})
    } catch (error) {
        next(error)
    }
}
...
exports.createGroup = async (req, res, next) => {
    try {
        ...
        res.status(200).json({status: true, message: 'Successfully fetched user', data: {group}})
    } catch (error) {
        next(error)
    }
}
...
exports.getGroupMessages = async(req, res, next) => {
    try {
        ...
        res.status(200).json({status: true, message: 'Successfully fetched messages', data: {messages: messages, totalItems: totalItems, group: group, status: 'group'}})
    } catch (error) {
        next(error)
    }  
} 
...
exports.createGroupMessage = async (req, res, next) => {
    try {
        ...
        for (const roomId of roomIds) {
            getIo().to(roomId).emit('receive-msg', {message: message, type: 'GROUP'})
        }
...
exports.deleteGroup = async (req, res, next) => {
    try{
        ...
        res.status(200).json({status: true, message: 'Successfully deleted group', data: null})
    } catch (error) {
        next(error)
    }  
}
