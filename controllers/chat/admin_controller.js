...
exports.getUsers = async (req, res, next) => {
    try {
        ...
        res.status(200).json({status: true, message: 'Fetched users successfully.', data: {users: users}})
    } catch (error) {
        next(error)
    }
}
...
exports.userDetail = async (req, res, next) => {
    try {
        ...
        const groups = await Group.find({creator: id}).countDocuments()
        const group_messages = await GroupMessage.find({user: id}).countDocuments()
        const private_messages = await PrivateMessage.find({from: id}).countDocuments()
        const user = await User.findById(id)
        res.status(200).json({status: true, message: 'Fetched users successfully.', data: {user, groups, group_messages, private_messages}})
    } catch (error) {
        next(error)
    }
}
